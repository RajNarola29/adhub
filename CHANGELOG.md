...
* Chore: Bumped the Android toolchain to current stable releases - AGP 8.11.1 -> 9.3.1, Gradle 8.14 -> 9.7.1, Kotlin 2.2.20 -> 2.4.10, `compileSdk` 36 -> 37, and Java/JVM target 17 -> 21. Consuming apps building against this version now need `compileSdk >= 37` and JDK 21 available - AGP 9.x enforces the `compileSdk` requirement at build time (`checkDebugAarMetadata` fails otherwise) rather than just warning about it.
* Chore: Bumped AppLovin (13.6.3.0 -> 13.6.4.0) and Unity (mediation adapter + `unity-ads`, 4.19.0.x -> 4.20.0.x) mediation adapters to their latest Android releases. `play-services-ads` and the Facebook adapter were already current. iOS CocoaPods for AppLovin/Unity remain one step behind Android (13.6.3.0 and 4.19.0.1) since Google/AppLovin/Unity haven't published the newer pods yet - `adhub.podspec` doesn't pin versions, so they'll resolve automatically once available.
* Fix: `ios/adhub.podspec`'s `s.version` had been stuck at `0.2.3` since the 0.2.4 release and silently drifted out of sync with `pubspec.yaml` for three releases. Now kept in sync.
* Fix (consumer action may be required): iOS builds on Flutter 3.47+ / Xcode 26+ can hit four build failures unrelated to any dependency version - documented here since they surfaced while verifying this release and consuming apps will likely hit the same ones: (1) `IPHONEOS_DEPLOYMENT_TARGET` below 15.0 fails `pod install` outright against this podspec's minimum; (2) a plain `use_frameworks!` in the app's Podfile conflicts with AppLovin/Meta/Unity's static xcframeworks - needs `use_frameworks! :linkage => :static`; (3) `onesignal_flutter` ships both a podspec and a `Package.swift`, and if the app's Xcode project has Flutter's SPM plugin integration wired in, Xcode embeds OneSignal's frameworks twice (`Multiple commands produce OneSignal*.framework`) - worked around here via `flutter config --no-enable-swift-package-manager` (a machine/CI-level toolchain setting, not something this package can fix from inside its podspec); (4) Xcode 26's stricter explicit-module builds reject a private header include inside Google's own `GoogleMobileAds.framework`, requiring `CLANG_ALLOW_NON_MODULAR_INCLUDES_IN_FRAMEWORK_MODULES = YES` in the app's Podfile `post_install` and `Debug.xcconfig`/`Release.xcconfig`.
